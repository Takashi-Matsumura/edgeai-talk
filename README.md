...
1. **RAGトグルボタン**: ヘッダー右上の青い楕円形ボタン（RAG）でON/OFF切り替え
   - **短押し/クリック**: RAG機能のON/OFF切り替え
   - **長押し（500ms以上）**: ドキュメント管理モーダルを開く
2. **音声トグルボタン**: 隣のオレンジ色の楕円形ボタン（音声）で音声機能のON/OFF切り替え

   **ドキュメント管理の機能:**
   - **2つの追加モード**:
     - 📁 **ファイルから追加**: .txt, .md, .pdfファイルをアップロード
     - ✏️ **テキストで追加**: エディタで直接テキストを編集・登録（展示会向け）

   **テキストエディタモード（展示会推奨）:**
   - テンプレート選択: 展示会ブース紹介、製品情報、イベントスケジュール
   - リアルタイムテキスト編集（Markdown対応）
   - ファイル名指定で即座にRAGに登録
   - 自動チャンク分割とベクトル化

   **ドラッグ&リサイズ対応:**
   - ヘッダーをドラッグしてモーダルを自由に移動
   - 右下をドラッグしてウィンドウサイズを変更
   - エディタエリアを最大限に拡大可能

   **その他の機能:**
   - ドキュメント一覧表示と内容プレビュー
   - ドキュメント削除
   - RAG統計情報の表示（ドキュメント数、チャンク数、ベクトル次元）
...
│   ├── routes/                           # APIルート
│   │   ├── documents.py                  # ドキュメント管理API（NEW: テンプレート・テキスト登録）
│   │   ├── rag.py                        # RAG検索API
│   │   └── chat.py                       # RAG対応チャットAPI
│   ├── templates/                        # 展示会向けテンプレート（NEW）
│   │   ├── booth_introduction.md         # ブース紹介テンプレート
│   │   ├── product_info.md               # 製品情報テンプレート
│   │   └── event_schedule.md             # イベントスケジュールテンプレート
...
### 展示会向けRAG強化機能（NEW - 2025-10-14）
- ✅ **テキストエディタモード**: テンプレートベースの直接編集機能
  - ✅ 3種類のテンプレート（ブース紹介、製品情報、イベントスケジュール）
  - ✅ リアルタイムMarkdownエディタ（文字数カウンター付き）
  - ✅ テキスト直接登録API（チャンク分割自動化）
- ✅ **ドラッグ&リサイズ対応モーダル**
  - ✅ ヘッダードラッグでウィンドウ移動
  - ✅ 右下ハンドルでサイズ変更（最小600×500px）
  - ✅ エディタエリア最大化レイアウト
- ✅ **コンパクトUI設計**
  - ✅ 統計情報・モード切り替えの横並び配置
  - ✅ テンプレート・ファイル名入力の2カラムレイアウト
  - ✅ テキストエディタに最大スペースを割り当て
...
- ✅ **シンプルトグルボタン（NEW - 2025-10-14）**
  - RAG: 青色楕円ボタン（短押しでON/OFF、長押しでドキュメント管理）
  - 音声: オレンジ色楕円ボタン（クリックでON/OFF）
  - 展開/折りたたみ不要のシンプルな操作性
